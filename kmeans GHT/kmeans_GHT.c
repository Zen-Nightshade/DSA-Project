...
bool Display_K_means_Tree(node*root){

    FILE *fptr = fopen("K_means_Tree.md","w");
...
int main(){
    ...
    double avg_split2_confidence =0;
    double avg_avg_split3 =0;
    double avg_split3_confidence=0;
    ...
    for(int i=0; i<n;++i){
        ...
        avg_split2_confidence += avg_split2/std_split2;
        ...
        avg_split3_confidence += avg_split3/std_split3;

        num_nodes += count_nodes+1;
        num_leaves += count_leaf;

        FILE * file = fopen("K_means_pivots.csv","w");
        if(file != NULL){
        ...
        check = Display_K_means_Tree(root);
        Free_tree(root);
        count_nodes=0;
        count_leaf=0;
        sum2_leaf=0;
    ...
    avg_std_dev_leaf /=n;
    avg_avg_split2 /= n;
    avg_split2_confidence /= n;
    avg_avg_split3 /= n;
    avg_split3_confidence /= n;
    ...
    printf("Average Split-2 quality= %f \nand sp2 confidence = %f\n\n", avg_avg_split2, avg_split2_confidence);
    printf("Average Split-3 quality= %f \nand sp3 confidence = %f\n\n", avg_avg_split3, avg_split3_confidence);
    for(int i=0; i<3;++i){
